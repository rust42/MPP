...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
		...
		return Stream.concat(
				...
				list2.stream().filter(a -> !list1.contains(a))
		).collect(Collectors.toList());
	}

	// Returns a list of those strings which belong to both the lists
	...
	private static void testThirdMethod() {
		List<String> list1 = Arrays.asList(new String[]{"friend", "tree", "bike", "bat"});
		List<String> list2 = Arrays.asList(new String[]{"tree", "strong", "bike", "weak"});
		assert(elementsInBoth(list1, list2).equals(List.of("tree", "bike")));
		System.out.println("Strings occurring in both list: " + elementsInBoth(list1, list2));
	}
}
