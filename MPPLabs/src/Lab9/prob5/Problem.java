...
//				.collect(Collectors.toList());
		...
		Comparator<Customer> customerComparator = Comparator.comparing(a -> a.getCity().length());
		Comparator<Customer> alphabeticalComparator =  Comparator.comparing(Customer::getCity);
		return list.stream()
				.sorted(customerComparator.thenComparing(alphabeticalComparator))
				.map(Customer::getZip)
				.collect(Collectors.toList());
	}
